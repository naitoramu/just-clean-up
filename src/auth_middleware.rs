...
pub async fn authorization_middleware(
    ...
    let auth_header = match auth_header {
        Some(header) => match header.to_str() {
            Ok(header) => header,
            Err(err) =>  return JsonProblems::forbidden(err.into()).into()
        },
        None => return JsonProblems::forbidden("Missing auth header".into()).into()
    };
    let mut header = auth_header.split_whitespace();
    let (_, token) = (header.next(), header.next());
    let token_data = match decode_jwt(JwtToken { access_token: token.unwrap().to_string() }) {
        Ok(claims) => claims,
        Err(err) => return JsonProblems::unauthorized(err).into()
    };
